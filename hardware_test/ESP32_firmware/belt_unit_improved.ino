 ...
 *
 * IMPROVED VERSION — posture-risk detection only. See belt_unit.ino for the
 * unmodified original. Changes are marked with "IMPROVEMENT:" comments.
 *
 * What changed and why (see chat/README for full explanation):
 *   1. Sustained duration  — posture must stay bad for POSTURE_SUSTAIN_MS
 *      before it's flagged, instead of alerting on a single 100ms sample.
 *   2. Hysteresis          — separate ENTER/CLEAR thresholds stop the flag
 *      from flickering when the angle hovers near the cutoff.
 *   4. Calibration-aware   — posture is not trusted (and never flagged)
 *      until the BNO055's system calibration (cal_sys) reaches a minimum
 *      quality, since freshly-booted IMUs report noisy/wrong orientation.
 *
 * NOT changed here — needs a physical step, not code (see point 5):
 *   MOUNT_PITCH_OFFSET must be verified by actually wearing the belt
 *   standing upright and checking what pitch reads (visible on the OLED
 *   IMU screen or Serial [IMU] log line already in this firmware). If
 *   standing-straight pitch isn't close to 0 degrees, adjust the offset
 *   below before trusting POSTURE_PITCH_ENTER/CLEAR at all — no amount of
 *   duration or hysteresis tuning fixes a mis-zeroed sensor.
 *
 * Wiring (XIAO ESP32S3):
 *   I2C SDA  → D4
 *   I2C SCL  → D5
...
// IMPROVEMENT (point 5, verify physically — not fixed by this file):
// Wear the belt standing upright and check the [IMU] Serial line or the OLED
// IMU screen. If pitch isn't close to 0 while standing straight, this offset
// is wrong for how the sensor is actually mounted, and POSTURE_PITCH_ENTER/
// CLEAR below will trigger on normal standing posture no matter how well
// the duration/hysteresis logic is tuned.
#define MOUNT_PITCH_OFFSET  90.0f

// ─── Detection thresholds ─────────────────────────────────────────────────────
// IMPROVEMENT (points 1 & 2): replaced the single ENTER-only threshold with
// an ENTER/CLEAR pair (hysteresis) plus a minimum sustained duration before
// alerting. See updatePostureRisk() below for how these are used together.
#define POSTURE_PITCH_ENTER       45.0f   // degrees — crossing this starts the "maybe at risk" timer
#define POSTURE_PITCH_CLEAR       35.0f   // degrees — must drop back below this to fully clear risk
#define POSTURE_ROLL_ENTER        30.0f   // degrees — lateral lean, enter threshold
#define POSTURE_ROLL_CLEAR        22.0f   // degrees — lateral lean, clear threshold
#define POSTURE_SUSTAIN_MS      3000      // must stay past ENTER continuously this long before alerting

// IMPROVEMENT (point 4): minimum BNO055 system calibration (0-3) required
// before posture readings are trusted enough to alert on.
#define POSTURE_MIN_CAL_SYS   1
...
struct ImuState {
  ...
  // IMPROVEMENT (points 1 & 2): tracks how long pitch/roll has continuously
  // been past the ENTER threshold. 0 means "not currently exceeding".
  unsigned long postureExceedStartMs;
};
...
// ─── Posture risk (IMPROVEMENT: points 1, 2, 4) ────────────────────────────────
// Sustained + hysteresis + calibration-aware. Replaces the original single-
// sample, single-threshold check. Called once per IMU read, right after
// imuState.pitch/roll/calSys are updated.
void updatePostureRisk() {
  bool exceedsEnter = (fabs(imuState.pitch) > POSTURE_PITCH_ENTER ||
                        fabs(imuState.roll)  > POSTURE_ROLL_ENTER);
  bool withinClear  = (fabs(imuState.pitch) < POSTURE_PITCH_CLEAR &&
                        fabs(imuState.roll)  < POSTURE_ROLL_CLEAR);

  if (exceedsEnter) {
    // Point 1: start (or continue) timing how long the bad angle has held.
    if (imuState.postureExceedStartMs == 0) {
      imuState.postureExceedStartMs = millis();
    }
    unsigned long heldMs = millis() - imuState.postureExceedStartMs;

    // Point 4: don't trust the angle at all until the IMU has calibrated.
    bool calibrated = imuState.calSys >= POSTURE_MIN_CAL_SYS;

    imuState.postureRisk = calibrated && (heldMs >= POSTURE_SUSTAIN_MS);

  } else if (withinClear) {
    // Point 2: only fully reset once back under the lower CLEAR threshold —
    // this is the hysteresis gap that stops flickering near ENTER.
    imuState.postureExceedStartMs = 0;
    imuState.postureRisk = false;
  }
  // else: pitch/roll is between CLEAR and ENTER (the hysteresis band) —
  // keep whatever state/timer we already had, don't reset and don't flag.
}
...
void readIMU() {
  ...
  // IMPROVEMENT: postureRisk is now computed by updatePostureRisk() instead
  // of a single-line instantaneous threshold check.
  updatePostureRisk();
