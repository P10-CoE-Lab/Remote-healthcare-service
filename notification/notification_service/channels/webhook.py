...
class WebhookChannel(Channel):
    ...
    async def send(
        ...
        resolved = provider_name or self._default_provider
        provider: Provider = self._registry.get(resolved)

        # Generic provider needs a webhook_url; validate before calling
        if provider.name == "generic":
            url = options.extra.get("webhook_url")
            if not url:
                return ChannelResult(
                    ...
                    error=NotificationError(
                        ...
                        message="Generic webhook provider requires 'webhook_url' in message payload",
                    ),
                )
            extra = {"url": url}
        else:
